 ...
 *
 * Attention: Since SQLite does not support row level locks but locks the whole database.php,
 * it means only one session can be accessed at a time. Even different sessions would wait
 * for another to finish. So saving session in SQLite should only be considered for
 * development or prototypes.
 *
 * Session data is a binary string that can contain non-printable characters like the null byte.
 * For this reason it must be saved in a binary column in the database.php like BLOB in MySQL.
...
class PdoSessionHandler implements \SessionHandlerInterface
{
     ...
     * lock is not enforced by the database.php and thus other, unaware parts of the
     ...
     * closing a session, you have to be careful when you use same database.php connection
     ...
     * Constructor.
     *
     * You can either pass an existing database.php connection as PDO instance or
     * pass a DSN string that will be used to lazy-connect to the database.php
    ...
    public function write($sessionId, $data)
    {
        ...
        try {
            // We use a single MERGE SQL query when supported by the database.php.
    ...
    /**
     * Lazy-connects to the database.php.
     *
     ...
     * on the database.php immediately. Because of https://bugs.php.net/42766 we have to create
    ...
    /**
     * Executes an application-level lock on the database.php.
     *
     * @param string $sessionId Session ID
     *
    ...
    /**
     * Returns a merge/upsert (i.e. insert or update) statement when supported by the database.php for writing session data.
     *
     * @param string $sessionId   Session ID
